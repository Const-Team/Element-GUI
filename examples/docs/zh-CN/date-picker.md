...
在选择日期时，可以配置默认要展示的面板，例如生日：希望默认展开的面板先选择年。

:::demo 需配置`picker-options`对象中的`defaultView`，参数可以为 `year/month`。
```html
<template>
  <el-date-picker
    v-model="value"
    ...
    type="date"
    placeholder="选择日期"
    :picker-options="{defaultView: 'year'}">
  </el-date-picker>
</template>
...
| type | 显示类型 | string | year/month/date/dates/ week/datetime/datetimerange/ daterange/monthrange daterange/monthrange/yearrange/quarter/quarterrange | date |
...
| defaultView | 设置默认展开的面板 | string | year,month,date | date |
