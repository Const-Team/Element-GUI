...
相对于饿了么自带的日历，此日历功能更加强大，支持农历、假日、可选区间等。

### 基本单选

:::demo 设置 `default-date` 来指定当前显示的月份。如果 `default-date` 未指定，则显示当月。
```html
<el-calendar-pro :default-date="defaultDate" :select-date="date"></el-calendar-pro>
...
:::demo 设置 `multiple` 开启日期多选。
```html
<el-calendar-pro multiple :select-date="date"></el-calendar-pro>
...
:::demo 设置 `first-day-of-week` 来指定周起始日。如果 `first-day-of-week` 未指定则按照周日为起始日。
```html
<el-calendar-pro :first-day-of-week="1" :select-date="date"></el-calendar-pro>
...
:::demo 设置 `highlighter-date` 高亮日期。
```html
<el-calendar-pro multiple :select-date="date" :highlighter-date="highlighter"></el-calendar-pro>
...
<script>
  export default {
    data() {
      return {
        date: [ "2019-09-07", "2019-10-10", "2019-10-16", "2019-10-15", "2019-10-22", "2019-10-18" ] ,
        highlighter: ["2019-12-24","2020-01-25","2020-02-13","2020-02-24","2020-02-26"]
      }
    },
  }
</script>
...
:::demo 设置 `disabled-date` 来指定当哪些日期不可选。
```html
<el-calendar-pro multiple :select-date="date" :disabled-date="disableddate"></el-calendar-pro>
...
:::demo 设置 `max-date` 和 `min-date` 来设置日期区间。
```html
<el-calendar-pro multiple  :select-date="date" max-date="2019-12-31" min-date="2019-05-01" :disabled-date="disableddate"></el-calendar-pro>
...
:::demo 内置 `render` 方法，参数为`year, month`，配合其他组件使用。另外，通过设置名为 `dateCell` 的 `scoped-slot` 来自定义日历单元格中显示的内容。在 `scoped-slot` 可以获取到 `date`（当前单元格的日期对象），详情解释参考下方的 `API` 文档。
```html
...
<el-calendar-pro ref="calendar" multiple :select-date="date"  :show-title="false">
