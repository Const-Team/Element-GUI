...
:::demo 节点默认和展开状态的图标可以通过 `icon-class` 和 `expand-icon-class` 进行设置。如果只设置 `icon-class` ，那么展开效果就是顺时针旋转90°，如果需要更换展开图标，需要配合`expand-icon-class` 进行设置。
...
<div class="custom-tree-container">
  ...
  <div class="block">
    <p>配合使用 expand-icon-class</p>
    <el-tree
      :data="data"
      node-key="id"
      icon-class="el-icon-circle-plus-outline"
      expand-icon-class="el-icon-remove-outline">
    </el-tree>
  </div>
</div>
