...
<template>
  <div class="headerWrapper" :class="{ishome:$route.name==='home'}">
    <header class="header" ref="header">
      <div class="container">
        ...
        <ul class="nav">
          ...
          <li class="nav-item">
            <router-link active-class="active" :to="`/guide`">{{ langConfig.guide }}</router-link>
          </li>
          <li class="nav-item">
            <router-link
              active-class="active"
...
export default {
  ...
  created() {
    ...
    xhr.send();
    let primaryLast = '#1890FF';
    bus.$on(ACTION_USER_CONFIG_UPDATE, val => {
      let primaryColor = val.global['$--color-primary'];
      if (!primaryColor) primaryColor = '#1890FF';
