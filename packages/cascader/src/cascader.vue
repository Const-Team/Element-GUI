  ...
  <div
    ...
    <div v-if="multiple" class="el-cascader__tags">
      <template v-if="collapseTags">
        <el-tag
          v-for="tag in presentTags.slice(0, collapseTagsMaxNum)"
          :key="tag.key"
          type="info"
          :size="tagSize"
          :hit="tag.hitState"
        ...
        <el-tag
          v-if="presentTags.length > collapseTagsMaxNum"
          :closable="false"
          :size="tagSize"
          class="tag-num"
          type="info"
          disable-transitions>
          <el-tooltip popper-class="cascader-tooltip" :max-width="tooltipMaxWidth" :effect="tooltipEffect" :content="presentTitle" placement="top">
            <span>+ {{ presentTags.length - collapseTagsMaxNum }}</span>
          </el-tooltip>
        </el-tag>
      </template>
      <template v-else>
...
import ElCascaderPanel from 'element-gui/packages/cascader-panel';
import AriaUtils from 'element-gui/src/utils/aria-utils';
import { t } from 'element-gui/src/locale';
import { isEqual, isEmpty, kebabCase, valueEquals } from 'element-gui/src/utils/util';
import { isUndefined, isFunction } from 'element-gui/src/utils/types';
import { isDef } from 'element-gui/src/utils/shared';
...
export default {
  ...
  mounted() {
    ...
    if (input && input.$el) {
      this.inputInitialHeight = input.$el.offsetHeight || InputSizeMap[this.realSize] || 34;
    }
  ...
  methods: {
    ...
    computePresentTags() {
      ...
      for (let i = 0; i < tags.length; i++) {
        title.push(tags[i].text);
      };
      this.presentTitle = title.join('，');
    },
    getSuggestions() {
