  ...
  export default {
    ...
    computed: {
      iconClass() {
        if(this.node.expanded && !this.node.isLeaf) {
          if (this.tree.iconClass && !this.tree.expandIconClass) {
            return this.tree.iconClass + ' expanded'
          };
          if (this.tree.expandIconClass) {
            return this.tree.expandIconClass
          } else {
            return 'el-icon-arrow-right expanded'
          }
        }
        return this.tree.iconClass ? this.tree.iconClass : 'el-icon-arrow-right'
      }
    },
    ...
    methods: {
      ...
      handleClick() {
        ...
        // 使disabled属性生效，在高亮属性为true时，不添加is-current
