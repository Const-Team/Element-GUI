...
export default {
  ...
  methods: {
    ...
    getCurrentDragPercentage(drag) {
      ...
      // 返回正整数
      return Math.floor(drag * 100 / containerSize);
    },
    ...
    checkSplitpanesNodes() {
      this.container.children.forEach(child => {
        const isPane = child.classList.contains('el-splitpanes__pane');
        const isSplitter = child.classList.contains('el-splitpanes__splitter');
        // Node is not a Pane or a splitter: remove it.
        if (!isPane && !isSplitter) {
          child.remove();
          console.warn('Splitpanes: Only <el-pane> elements are allowed at the root of <el-splitpanes>. One of your DOM nodes was removed.');
          return;
        }
      });
    },
    addSplitter(paneIndex, nextPaneNode, isVeryFirst = false) {
      ...
      elm.classList.add('el-splitpanes__splitter');
      if (!isVeryFirst) {
    ...
    redoSplitters() {
      ...
      children.forEach(el => {
        if (el.className.includes('el-splitpanes__splitter')) this.removeSplitter(el);
      });
      let paneIndex = 0;
      children.forEach(el => {
        if (el.className.includes('el-splitpanes__pane')) {
    ...
    onPaneAdd(pane) {
      // 1. Add pane to array at the same index it was inserted in the <el-splitpanes> tag.
      let index = -1;
  ...
  watch: {
    ...
    dblClickSplitter(enable) {
      const splitters = [...this.container.querySelectorAll('.el-splitpanes__splitter')];
  ...
  render(h) {
    return h(
      'div',
      {
        ...
        class: [
          'el-splitpanes',
          `el-splitpanes--${this.horizontal ? 'horizontal' : 'vertical'}`,
          {
            'el-splitpanes--dragging': this.touch.dragging
          }
        ]
      },
      this.$slots.default
